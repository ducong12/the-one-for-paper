...
public class SimScenario implements Serializable {
	...
	public static final String NROF_USERS_S = "nrofUsers";
	public static final String USER_NS = "User";
	public static final String AFFILIATION_S = "affiliation";
	public static final String CITY_S = "city";
	public static final String NATIONALITY_S = "nationality";
	...
	protected SimScenario() {
		...
		nrofUsers = s.getInt(NROF_USERS_S);
		this.diffUsers=new int[nrofUsers];
	...
	protected void createHosts() {
		...
		for (int i=1; i<=nrofGroups; i++) {
			...
			for (int j=0; j<nrofHosts; j++) {
				...
				int index=(int)(Math.random()*78);
				while (diffUsers[index]==1){
					index=(int)(Math.random()*78);
				}
				diffUsers[index]=1;
				Settings s1 = new Settings(USER_NS+index);
				String affiliation = s1.getSetting(AFFILIATION_S);
				String city = s1.getSetting(CITY_S);
				String nationality = s1.getSetting(NATIONALITY_S);
				String language = s1.getSetting(LANGUAGE_S);
				String country = s1.getSetting(COUNTRY_S);
				String position = s1.getSetting(POSITION_S);

				// prototypes are given to new DTNHost which replicates
				// new instances of movement model and message router
				DTNHost host = new DTNHost(this.messageListeners, 
						this.movementListeners,	gid, interfaces, comBus, 
						mmProto, mRouterProto, affiliation, city, nationality,
						language, country, position);
				hosts.add(host);
			}
		}
	}
